...
function sensorData(t, data) {
  const categoryData = data.reduce((map, value) => {
    const index = parseInt(value.sensorIdentifier);
    map[index] = map[index] || 0;
    map[index] = map[index] + parseInt(value.activationCount);
    return map;
  }, {});

  const categories = Object.keys(categoryData).sort((a, b) => a - b);
    ...
    toolbox: {
      ...
      feature: {
        dataView: {
          ...
          readOnly: true,
          optionToContent(opt) {
            const axisData = opt.series[0].data;
